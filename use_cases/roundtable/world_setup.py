...
def construct_agent(agent_data, base_agent_data, base_kwargs):
    ...
    for k in set(agent_data) | set(base_agent_data):
        if (
            k in agent_data
            and agent_data[k] != None
            and k in base_agent_data
            and base_agent_data[k] != None
        ):
    ...
    # Filter the dictionary to only include keys that correspond to the class constructor's parameters
    filtered_agent_data = {
        k: v for k, v in combined_agent_data.items() if k in arg_names
    }
...
def construct_world(data):
    ...
    if "path_to_external_memory" in world_def:
        personality_db_qdrant_client = QdrantClient(
            path=os.path.join(ABS_PATH, world_def["path_to_external_memory"])
        )
    ...
    ]  # Assuming you have a construct_agent function
...
def launch_use_case(
    world_definition="default_world_definition.yaml",
