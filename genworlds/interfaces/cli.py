...
class ChatRoom:
    ...
    def process_event(self, ws_json_message):
        # router for event types modify for menu items
    ...
    def _process_world_state(self, message):
        ...
        self.menu_items["All Events"] = {"buffer": Buffer(), "event_types": ["all"]}

        # Specific for the podcast world TODO: generalize changing for location events
        if message["world_type"] == "PodcastWorld":
            self.menu_items["Podcast Room"] = {
                "buffer": Buffer(),
        ...
        # Update the menu buffer to display the new menu items
        self.menu_buffer.text = self._create_menu_text()
