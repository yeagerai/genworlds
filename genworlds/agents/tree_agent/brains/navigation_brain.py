...
class NavigationBrain:
    LLMParams = TypedDict('LLMParams', {
        ...
        "relevant_commands": list[str],  # just the event types, and the description of what the event does (not even the name)
        "messages": list, 
    })

    def __init__(
        self, openai_api_key: str, ai_name: str, model_name="gpt-3.5-turbo", search_algorithm="BFS"
    ):
        ...
        self.gen_prompt = NavigationGeneratorPrompt(
            ...
            ai_role="You are {ai_name}. You need to choose your next action that helps you achieve your goals. It must be consistent with all of the following information:".format(ai_name=ai_name),
        ...
        self.eval_prompt = NavigationGeneratorPrompt(
            ...
            ai_role="You are {ai_name}, an expert in evaluating which events get you closer to achieving your goals based on:".format(ai_name=ai_name),
