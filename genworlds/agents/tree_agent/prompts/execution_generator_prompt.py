...
class ExecutionGeneratorPrompt(BaseChatPromptTemplate, BaseModel):
    ...
    def format_messages(self, **kwargs: Any) -> List[BaseMessage]:
        ...
        if "previous_brain_outputs" in kwargs and len(kwargs["previous_brain_outputs"]) > 0:       
            ...
            previous_brain_outputs_message = SystemMessage(content=previous_brain_outputs_prompt)
        ...
        if "command_to_execute" in kwargs:
            command_to_execute_message = SystemMessage(content=f"## You have selected the following command to execute:\n{kwargs['command_to_execute']}")
        ...
        if "personality_db" in kwargs and kwargs["personality_db"] is not None:
            ...
            past_statements = list(
                map(
                    ...
                    personality_db.similarity_search(kwargs['previous_brain_outputs'][-1]),
                )
            )
        ...
        relevant_docs = memory.get_relevant_documents(kwargs['previous_brain_outputs'][-1])
